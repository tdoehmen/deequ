  ...
  def getStats(): ExtendedMultivariateStatistics = {
    val hashingColumnsLookups = configuration.frequentItems match {
      case true => Some(longsSketches.map( sketch => {
        ...
        val items_lookup = items.take(Math.min(items.length, 255)).map(kv => (kv.getItem, kv
          .getEstimate))
        items_lookup
      }))
      case false => None
...
case class ExtendedMultivariateStatistics(mean: Vector,
                                          ...
                                          freqItems: Option[IndexedSeq[Array[(Long, Long)]]])

case class ExtendedStatsConfig(approxDistinctness: Boolean = true,
                       frequentItems: Boolean = true, freqItemNumber: Int = 100,
